...
const Filter = () => {
   ...
   <div className='w-64 hidden tablet:block '>
    ...
    <Map />
      ...
      <h1 className='text-lg font-semibold mb-2'>Search by property name</h1>
             <Menu>
            <MenuButton as={Button} w="240px" h="46px" bg="white"  border="1px solid black">
            <div className='flex justify-start  items-center gap-3' >
                    <div><AiOutlineSearch size="24px" /></div>
           ...
           <MenuList mt="-56px" minW="240px">
              ...
              <input type="text" placeholder='Search by Name' style={{width:"200px",padding:"4px",fontSize:"16px",fontWeight:"bold",outline:"none"}} ref={ref} value={query} onChange={handleInput} />
               </div>
               <div className='w-62 rounded-md z-50'>
                       {
                        searchdata.length!==10 && searchdata.map((items)=>(
                           <Link to={`/product`}>
                           <div key={items._id}className="w-62 px-3 py-1 mt-2 hover:bg-background hover:text-black rounded-sm overflow-hidden" onClick={(()=>handleSet(items.title))} >{items.aboutProperty.title}</div>
                           </Link> 
                        ))
                      }
               </div>
  ...
  <div>
    ...
    <h3 className='text-md font-semibold mb-2'>Amenities</h3>
          ...
          <div className='flex justify-start gap-4'>
            <input type="checkbox" style={{padding:"3px"}} value="Dinner" defaultChecked={category.includes("Dinner")}  onChange={handleCategoryChange} />
            <label className='text-md font-medium'>Dinner</label>
          </div>
          <div className='flex justify-start gap-4'>
            <input type="checkbox" style={{padding:"3px"}} value="Lunch" defaultChecked={category.includes("Lunch")}  onChange={handleCategoryChange} />
          ...
          <div className='flex justify-start gap-4'>
            <input type="checkbox" style={{padding:"3px"}} value="FreeWiFi" defaultChecked={category.includes("FreeWiFi")}  onChange={handleCategoryChange}  />
            <label className='text-md font-medium'>Free WiFi</label>
          </div>
          <div className='flex justify-start gap-4'>
            <input type="checkbox" style={{padding:"3px"}} value="FreeParking" defaultChecked={category.includes("FreeParking")}  onChange={handleCategoryChange}  />
